const THINGSPEAK_URL = 'https://api.thingspeak.com/channels/2936641/feeds.json?api_key=98ZPQ930QYNNI2A9&results=1';
   ...
   function simulateWorkerDetails(role) {
       switch (role.toLowerCase()) {
           case 'engineer': return { age: 30, workingLevel: 'High' };
           case 'supervisor': return { age: 45, workingLevel: 'Medium' };
           case 'visitor': return { age: 55, workingLevel: 'Low' };
           default: return { age: 'N/A', workingLevel: 'N/A' };
       }
   }
   let gauges = {
       temp: new JustGage({
           ...
           valueFontColor: "#fff", titleFontColor: "#fff", labelFontColor: "#fff"
       }),
       hum: new JustGage({
   ...
   function updateGaugeColors() {
       ...
       for (let gauge in gauges) {
           gauges[gauge].refresh(gauges[gauge].config.value, 0, {
   ...
   async function fetchData() {
       try {
           ...
           let workerDetails = simulateWorkerDetails(readings.role);
   ...
   setInterval(() => updateSuggestions(lastReadings || { role: "N/A", age: "N/A", workingLevel: "N/A" }), 5000);
   fetchData();
   updateGaugeColors();